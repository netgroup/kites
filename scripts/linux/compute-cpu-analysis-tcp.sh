...
for i in "${!files[@]}"; do
    ...
    echo ${tcp_configs[$tcp_config]}
        ...
        do
            ...
            if ( [[ ${configs_names[$config]} == "SAMEPOD" ]] && [[ ${tcp_configs[$tcp_config]} == "POD" ]] ) || ([[ ${configs_names[$config]} == "SAMENODE" ]]); then
                for (( minion_n=1; minion_n<=$N; minion_n++ ))
                do
                    ...
                    if [ -s temp_minion.csv ]; then
                        ...
                        tcp_thr=$(awk -F',' '$20=='$config' && $4 ~ /'k8s-minion-$minion_n'/ {sum+=$14; ++n} END { print sum/n }' temp_tcp_config.csv)
                ...
                do
                    ...
                    do
                        if [ $m_j -ne $m_i ]; then
                            ...
                            tcp_thr=$(awk -F',' '$20=='$config' && $4 ~ /'k8s-minion-${m_i}'/ && $5 ~ /'k8s-minion-${m_j}'/ {sum+=$14; ++n} END { print sum/n }' temp_tcp_config.csv)
                            # sleep 30
