...
vagrant ssh k8s-master-1.k8s-play.local -- -t '/vagrant/ext/kites/scripts/linux/kites.sh --cni weavenet --nodes 2'
# CALICO IPIP
echo -e "\n ###-----> VAGRANT PROVISION CLEAN UP <-----###\n"
vagrant provision --provision-with cleanup
echo -e "\n ###-----> WAIT 60 SECONDS <-----###\n"
...
vagrant ssh k8s-master-1.k8s-play.local -- -t '/vagrant/ext/kites/scripts/linux/start.sh --cni calicoIPIP --nodes 2'
# CALICO VXLAN
echo -e "\n ###-----> VAGRANT PROVISION CLEAN UP <-----###\n"
vagrant provision --provision-with cleanup
echo -e "\n ###-----> WAIT 60 SECONDS <-----###\n"
...
vagrant ssh k8s-master-1.k8s-play.local -- -t '/vagrant/ext/kites/scripts/linux/start.sh --cni calicoVXLAN --nodes 2'
# FLANNEL
echo -e "\n ###-----> VAGRANT PROVISION CLEAN UP <-----###\n"
vagrant provision --provision-with cleanup
echo -e "\n ###-----> WAIT 60 SECONDS <-----###\n"
...
vagrant ssh k8s-master-1.k8s-play.local -- -t '/vagrant/ext/kites/scripts/linux/start.sh --cni flannel --nodes 2'
