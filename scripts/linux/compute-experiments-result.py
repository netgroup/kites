    ...
    for minion_i in range(1, N+1, 1):
        for minion_j in range(1, N+1, 1):
            if minion_i != minion_j:
                string_name='k8s-minion-'+ str(minion_i) +'TOk8s-minion-'+ str(minion_j) +''
                print(string_name)
                row_conf = pps_df[ pps_df['TEST_TYPE'] == 'k8s-minion-'+ str(minion_i) +'TOk8s-minion-'+ str(minion_j) +'' ].index
...
diffnode = pd.DataFrame(rows, columns=["PPS", "rx/tx", "txed/totx", "cpu_tx", "cpu_rx"])
print(diffnode)